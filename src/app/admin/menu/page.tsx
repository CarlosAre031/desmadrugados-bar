        ...
        <div className="px-4 pb-4 border-t border-border">
          <div className="mt-3 space-y-2">
            {categories.map((cat) => (
              <div key={cat.id} className="flex items-center gap-2">
                {editingId === cat.id ? (
                  <>
                    <input className="flex-1 bg-background border border-[#D4A017] rounded px-2 py-1.5 text-sm outline-none" value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })} placeholder="Emoji" style={{ maxWidth: 60 }} />
                    <input className="flex-1 bg-background border border-[#D4A017] rounded px-2 py-1.5 text-sm outline-none" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Nombre ES *" />
                    <input className="flex-1 bg-background border border-border rounded px-2 py-1.5 text-sm outline-none focus:border-[#D4A017]" value={form.nameEn} onChange={(e) => setForm({ ...form, nameEn: e.target.value })} placeholder="Name EN" />
                    <button onClick={saveEdit} className="p-1.5 bg-[#D4A017] text-black rounded hover:bg-[#C96A1A] transition-colors"><Check size={14} /></button>
                    <button onClick={() => setEditingId(null)} className="p-1.5 border border-border rounded hover:border-[#B83232] hover:text-[#B83232] transition-colors"><X size={14} /></button>
                  </>
                ) : (
                  <>
                    ...
                    <button onClick={() => startEdit(cat)} className="p-1.5 border border-border rounded hover:border-[#D4A017] hover:text-[#D4A017] transition-colors"><Pencil size={13} /></button>
                    <button onClick={() => deleteCategory(cat.id)} className="p-1.5 border border-border rounded hover:border-[#B83232] hover:text-[#B83232] transition-colors"><Trash2 size={13} /></button>
                  </>
                )}
              </div>
            ))}
          ...
          {newCat ? (
            <div className="flex items-center gap-2 mt-3 pt-3 border-t border-border">
              <input className="bg-background border border-border rounded px-2 py-1.5 text-sm outline-none focus:border-[#D4A017]" value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })} placeholder="Emoji" style={{ maxWidth: 60 }} />
              <input className="flex-1 bg-background border border-border rounded px-2 py-1.5 text-sm outline-none focus:border-[#D4A017]" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Nombre ES *" />
              <input className="flex-1 bg-background border border-border rounded px-2 py-1.5 text-sm outline-none focus:border-[#D4A017]" value={form.nameEn} onChange={(e) => setForm({ ...form, nameEn: e.target.value })} placeholder="Name EN" />
              <button onClick={addCategory} className="p-1.5 bg-[#D4A017] text-black rounded hover:bg-[#C96A1A] transition-colors"><Check size={14} /></button>
              <button onClick={() => { setNewCat(false); setForm({ name: '', nameEn: '', icon: '' }) }} className="p-1.5 border border-border rounded hover:border-[#B83232] hover:text-[#B83232] transition-colors"><X size={14} /></button>
            </div>
          ) : (
            <button
              onClick={() => { setNewCat(true); setForm({ name: '', nameEn: '', icon: '' }) }}
              className="flex items-center gap-2 mt-3 pt-3 border-t border-border text-sm text-muted-foreground hover:text-[#D4A017] transition-colors"
            >
